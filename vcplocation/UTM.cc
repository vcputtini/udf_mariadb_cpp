...
/*!
 * \note ORIGINAL
 * \author Original Javascript by Chuck Taylor
 * \details Port to C++ by Alex Hajnal
 ...
 * FLOAT_64 (see below)
 *
 * This is a simple port of the code on the Geographic/UTM Coordinate
 * Converter (1) page from Javascript to C++.
 * Using this you can easily convert between UTM and WGS84 (latitude and
 * longitude). Accuracy seems to be around 50cm (I suspect rounding errors are
 * limiting precision). This code is provided as-is and has been minimally
 * tested; enjoy but use at your own risk! The license for UTM.cpp and UTM.h is
 * the same as the original Javascript: "The C++ source code in UTM.cpp and
 * UTM.h may be copied and reused without restriction."
 *
 * 1) http://home.hiwaay.net/~taylorc/toolbox/geography/geoutm.html
 *
 * ============================================================================
 * ChangeLog: Volnei Cervi Puttini <vcputtini@gmail.com> March'22
 * + Minor fixes.
 *
 * ChangeLog: Volnei Cervi Puttini <vcputtini@gmail.com> April'20
 *
 * + Usage license remains the same as the original source.
 * + All code marked with 'VCP' was adapted by me.
 * + For reasons of standardization and better identification of the code, we
 *   encapsulate the functions within the UTM class, thereby facilitating
 *   integration with our code.
 * + Standardized to 64bit accuracy only.
 * + Added std:: namespace identification in all necessary cases.
 * + Tested C++ compilers: c++11 or greater.
 *
 */
#include "UTM.h"
...
FLOAT
UTM::ArcLengthOfMeridian(FLOAT phi)
{
  ...
  /* Precalculate alpha */
  alpha =
    ((sm_a + sm_b) / 2.0) * (1.0 + (POW(n, 2.0) / 4.0) + (POW(n, 4.0) / 64.0));

  /* Precalculate beta */
  beta =
    (-3.0 * n / 2.0) + (9.0 * POW(n, 3.0) / 16.0) + (-3.0 * POW(n, 5.0) / 32.0);
...
FLOAT
UTM::UTMCentralMeridian(int zone)
{
  FLOAT cmeridian;
...
void
UTM::MapLatLonToXY(FLOAT phi, FLOAT lambda, FLOAT lambda0, FLOAT& x, FLOAT& y)
{
...
void
UTM::MapXYToLatLon(FLOAT x, FLOAT y, FLOAT lambda0, FLOAT& phi, FLOAT& lambda)
{
  ...
  /* Calculate latitude */
  phi = phif + x2frac * x2poly * (x * x) + x4frac * x4poly * POW(x, 4.0) +
...
int
UTM::LatLonToUTMXY(FLOAT lat, FLOAT lon, int zone, FLOAT& x, FLOAT& y)
{
  if ((zone < 1) || (zone > 60)) {
